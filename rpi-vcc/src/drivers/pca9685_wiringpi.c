...
void pca9685_setmk(int fd, int channel, int left, int right)	//设置指定通道的脉宽。fd是在pca9685_init时获得的设备描述符，num是通道号（从0开始），mk是脉宽单位是us。周期已经固定为20ms了
{
	//fd = wiringPiI2CSetup(0x70);
	unsigned int ON, OFF;
	//ON = 0;	//每次周期一开始就输出高电平
	ON = (unsigned int)((((double)left) / 20000 * 4096)*1.0067114);
	OFF = (unsigned int)((((double)right) / 20000 * 4096)*1.0067114);	//最后的1.0067114是校准用的
	//    printf("off = 0x%x", OFF);
